...
BINARY="${SCRIPT_DIR}/optimized_parallel_kmeans"
RESULTS_DIR="${PROJECT_ROOT}/results"
MAX_ITERATIONS="${MAX_ITERATIONS:-20}"
CSV_PATH="${RESULTS_DIR}/Optimized_Parallelism_Results.csv"
TXT_PATH="${RESULTS_DIR}/Optimized_Parallelism_Results.txt"
...
for dataset in "${datasets[@]}"; do
    ...
    for thread_count in "${threads[@]}"; do
        echo "Running Optimized Parallelism on ${dataset} with ${thread_count} thread(s)..."
